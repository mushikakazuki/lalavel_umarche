                          ...
                          @foreach ($expiredOwners as $owner)
                            <tr>
                              ...
                              <form id="delete_{{ $owner->id }}" method="POST" action="{{ route('admin.expired-owners.destroy', ['owner' => $owner->id]) }}">
                                @csrf
                                <td class="px-4 py-3">
                                  <a href="#" data-id="{{ $owner->id }}" onclick="deletePost(this)" class="flex text-white bg-red-300 border-0 py-2 px-4 focus:outline-none hover:bg-red-400 rounded">完全に削除</a>
                                </td>
                              </form>
                            </tr>
                          @endforeach
